...
import { IsDateString, IsNotEmpty, IsString, Length, ValidateIf } from "class-validator";
...
export class CreateLeaveRequestDto {
    ...
    @Transform(({ value }) => {
        ...
        if (isBefore(new Date(value), new Date()) && !isToday(new Date(value))) throw new BadRequestException('Leave from date cannot be in the past');
