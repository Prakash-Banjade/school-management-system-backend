...
export class OnlineClassesController {
  ...
  updateStatus(@Param('id') id: string, @Body() dto: UpdateOnlineClassStatusDto) {
    return this.onlineClassesService.updateStatus(id, dto);
  }

  @Patch(':id')
  @CheckAbilities({ subject: Role.TEACHER, action: Action.UPDATE })
  update(@Param('id') id: string, @Body() dto: UpdateOnlineClassDto) {
    return this.onlineClassesService.update(id, dto);
  }
